...
The project is scaffolded and under active development: Vite + React + TypeScript, Tailwind CSS v4, Vitest, and ESLint (see `package.json` for the full script list — `npm run dev`, `npm run build`, `npm run lint`, `npm run test:run`). Simple Mode's Workload Data and Backup Repository Configuration cards are implemented; Advanced Mode does not exist yet.
...
- **SOBR (Scale-Out Backup Repository) is the core domain complexity**: a three-tier model (Performance / Capacity / Archive), where each tier has a different set of allowed target types and its own rules (e.g. Capacity tier supports Copy vs Move modes with an offload period; Archive tier offloads on a "move backups older than N days" threshold — phrased in GFS terms in the UI ("Move GFS archives older than: N days") but modeled and validated as a flat day count, matching `VmAgentInputs.archiveTierDays`, not a Weekly/Monthly/Yearly picker). See the project brief for the exact allowed-targets matrix per tier — don't re-derive it from memory, it's easy to get subtly wrong.
- **Two relational validation rules on top of that matrix are also easy to re-derive wrong from memory** — each has an ADR recording a wrong first draft. (1) Archive-Feed compatibility: whichever tier sits immediately upstream of Archive Tier (Capacity's type if enabled, else Performance's) must support feeding it — Google Cloud is disqualified from that role unconditionally, it's one rule, not separately-gated "direct" vs "offload" paths (ADR-0012). (2) Vault Minimum Retention: any Vault-typed location (Turnkey target, Primary in Copy mode, SOBR Performance/Capacity) must retain data ≥30 days (`VAULT_MINIMUM_RETENTION_DAYS`), and each GFS class's lifetime is chain-completion-aware, not a naive `count × period` (ADRs 0013–0015). See `CONTEXT.md` for the "Archive-Feed Source," "Driving Cadence," and "Vault Minimum Retention" glossary terms, and `src/lib/simple-mode/vault-retention.ts` for the residency math.
