...
We do no local sizing computation. `veeam-vault-sizer` POSTs a `VmAgentRequest` to Veeam's existing public sizing engine (`https://calculator.veeam.com/vse/api/VmAgent`) — via a Cloudflare Pages Function proxy in production and a Vite dev-server proxy locally — and renders whatever `VmAgentResponse` comes back. This mirrors the sibling project `vdc-vault-readiness`, which already integrates the same endpoint and request/response contract.
