...
- **D2 — Response contract is a discriminated union of two un-mutilated `CVmAgentReturnObject`s** (not the flattened `primaryStorageMetrics`/`primaryProxyCompute` field names from the architectural brief that opened this brainstorm — those strings are brainstorm shorthand and do not appear in the committed `veeam_vault_sizer_project_brief.md`). `CVmAgentReturnObject` already bundles storage, compute, and telemetry per site; two of them _is_ the primary/secondary split, and the existing display components reuse verbatim.
...
- **D7 — Both bandwidth rows on both sides.** Nightly Incremental (8h) _and_ Initial Full / Restore (24h) render on Primary and Secondary. Initial-full visibility on Primary matters for local RTO validation, not just the nightly window. Windows come from the existing `BACKUP_WINDOW_HOURS` / `FULL_BACKUP_WINDOW_HOURS` constants.
  ...
  - **Direction is repo-facing, not vault-specific.** `NetworkBandwidth` displays `outboundMBps` — the proxy→repository write rate. That is the meaningful direction for _any_ repository type (vault or on-prem block/file), not just a vault, and the component's rendered labels ("Nightly Incremental", "Initial Full / Restore") carry no "vault" wording (the "vault-facing" phrasing lived only in commit messages). So verbatim reuse in the Primary section is semantically correct, not merely visually — the "vault-facing" framing was an artifact of the single-target case always being a vault.
  - **Shared `initialFullRestore` is a defensible approximation for both sides.** It is derived once from `workloadData` via `calculateInitialFullBandwidth`, whose `outboundMBps = (sourceTB ÷ 24h) × (1 − reduction)` — i.e. the _reduced_ dataset transferred within the full-backup window. That reduced dataset is exactly what moves in both cases: the reduced write landing on the Primary, and the Primary→Secondary copy seed (a copy transfers already-reduced backup data, not raw source). Nightly-incremental figures, by contrast, are each side's own per-pipeline `proxyCompute.networkThroughput` from the upstream, so they are not shared. **Assumption flagged for SE confirmation** (see Assumptions below): the copy seed and the local ingest are treated as the same reduced-dataset magnitude over the same 24h window.
...
- **D10 — Both pipelines size as independent backup targets; the calculator's copy flags stay at `base` defaults (`backupType: 0`, `copiesEnabled: false`).** Verified empirically against the live VmAgent endpoint (2026-07-15): toggling `backupType` (0/1) and `copiesEnabled` (true/false) produces **byte-identical** responses across every field — `totalStorageTB`, `workspaceGB`, immutability tax, proxy cores/RAM, network throughput (inbound and outbound), repo volumes, and restore-point count — for **both** an object-storage (Vault) and a block/file (Hardened Repository) config. So these two flags are inert for this endpoint; setting `backupType: 1` on the Secondary would not change its numbers, and `copiesEnabled: true` would not double-count. `buildBaseInputs` therefore keeps both flags at their existing defaults for both sub-requests — no per-side flag threading. Evidence is committed and re-runnable at `docs/evidence/copy-flags-inert/` (probe script + raw response fixtures + a README); the probe exits non-zero if the upstream ever starts honoring these flags, so the claim stays falsifiable rather than living only in this doc's prose.

## Assumptions to confirm with SEs

These are deliberate consequences of the decisions above, not defects — recorded so a reviewer can veto them against real-world SE expectations before or during implementation.

- **Default topology sizes ~2× (two near-identical pipelines).** Because D1 defaults _both_ sides to the full `workloadData` retention + GFS, an untouched copy configuration produces a Primary and a Secondary of roughly equal size, so "Combined Required Storage" is roughly double the source footprint. This is correct given the defaults, but real copy topologies usually differ Primary vs. Secondary (e.g. short local retention, long GFS offsite). The per-side retention overrides (`primary.retention`, `secondaryRetention`) exist precisely to express that; the question is only whether the _default_ should stay symmetric. Current decision: keep it symmetric (each side independently defaults to `workloadData`); the user drives asymmetry via the override blocks.
- **Copy seed ≈ local ingest for the Initial Full / Restore row.** See D7 — the shared `initialFullRestore` treats the Primary→Secondary copy seed and the local source ingest as the same reduced-dataset magnitude over the same 24h window. Defensible (a copy moves already-reduced data), but the WAN transfer window/characteristics could differ from a LAN ingest; flagged in case SEs want a distinct Secondary figure later.
