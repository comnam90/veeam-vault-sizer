...
interface RepositoryConfigValues {
  ...
  targetRepositoryImmutableDays: string; // active iff targetRepository !== 'sobr' (turnkey Vault Azure/AWS); regardless of backupPath
...
`primary.retention` is nested inside `PrimaryRepositoryConfig` while `secondaryRetention` sits flat at the top level. That asymmetry is a deliberate consequence of the reuse decision above, not an oversight: Secondary's repo-type config _is_ the top-level `targetRepository`/`sobr`/`targetRepositoryImmutableDays` fields (shared with Direct mode), so `secondaryRetention` is the only Secondary-specific field and has nothing to nest inside. Primary has no such shared parent — its type, immutability, and retention all belong to it alone — so they nest together in `PrimaryRepositoryConfig`.
...
### Why Archive Tier's offload trigger is a day count, not a GFS-point picker

CLAUDE.md's domain summary describes the Archive Tier as offloading "by GFS point," which could read as a UX asking the SE to pick a specific Weekly/Monthly/Yearly point rather than type a number. This spec keeps the field as a raw day count — "Move GFS archives older than: `[ 90 ]` days" — for two reasons: it maps 1:1 onto `archiveTier.moveDays` with no translation layer, and the already-built API layer's `VmAgentRequest.archiveTierDays` is itself a flat day count, so a GFS-point picker would need to convert back to a number before it could ever reach the request anyway. CLAUDE.md will be updated to match this implementation reality rather than the other way around.

### Why `targetRepositoryImmutableDays` exists as its own top-level field

An earlier version of this spec collected immutability for `primary.repoType` and every `SobrConfig` tier, but not for the turnkey `targetRepository: 'vault-azure' | 'vault-aws'` path — the most common flow in `screen.png`'s defaults. That gap wasn't intentional: `VmAgentResponseData.capacityTierImmutabilityTaxGB` confirms immutability is a real, sizing-relevant, SE-configurable input in the actual API — not a fixed or Veeam-managed constant — so every context where a Vault (or other immutability-requiring) type can be selected needs to collect it, including the turnkey path. `targetRepositoryImmutableDays` fills that gap. It sits at the top level rather than nested under `sobr`, because the turnkey path has no other config object of its own — `targetRepository` alone already fully describes a turnkey choice.
...
- **`backup-repository-card.tsx`**: controlled `{ value: RepositoryConfigValues; workloadData: WorkloadDataValues; onChange }`. Renders the Backup Path toggle and the three target cards. When `targetRepository` is `'vault-azure'` or `'vault-aws'`, renders an inline `Immutable for: [ ] days` field beneath the target cards, bound to `targetRepositoryImmutableDays`; hidden when `targetRepository === 'sobr'`, since SOBR manages immutability per tier instead. Stacks the Primary track above the Secondary block only when `backupPath === 'copy'`. Delegates repo-type selection to `repo-type-picker.tsx`, retention hatches to `retention-override-block.tsx`, and the tier pipeline to `sobr-builder.tsx` when `targetRepository === 'sobr'`.
...
- **Backup Path toggle** and the three target cards (Vault Azure, Vault AWS, SOBR Builder) render as in the reference mockup — a segmented toggle and emerald-bordered selectable cards with a check indicator. Selecting Vault Azure or Vault AWS reveals an inline immutability field beneath the cards (new — see the Data Model section); selecting SOBR Builder does not, since immutability is configured per tier instead.
...
- **SOBR Design Block**: Performance Tier's picker shows all three categories (Vault, On-Prem, Cloud Object Storage — 8 types total). Capacity Tier's picker shows two categories (Vault, Cloud Object Storage — 5 types, no NAS). Archive Tier's `+ Add` affordance is always clickable, independent of Capacity Tier's enabled state. The three tier rows use the existing `tier-performance`/`tier-capacity`/`tier-archive` design tokens (green/blue/gray) for their row accents and headings — CLAUDE.md calls this color-coding load-bearing and consistent throughout the UI, so it applies here exactly as it does everywhere else tiers appear.
...
| `targetRepositoryImmutableDays`         | `'30'` (not shown until `targetRepository` is a turnkey Vault choice)    |
...
| Field                                                       | Trigger                                                                                | Rule                                                                                                                                                                                                                                       |
| ----------------------------------------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `targetRepositoryImmutableDays`                             | `targetRepository !== 'sobr'`                                                          | integer ≥ 1                                                                                                                                                                                                                                |
...
| `sobr.capacityTier.moveDays`                                | `targetRepository === 'sobr'` AND `capacityTier.enabled` AND `capacityTier.movePolicy` | integer ≥ 1                                                                                                                                                                                                                                |
...
**`capacityTier.moveDays` floor corrected to ≥ 1** (was ≥ 0 in an earlier draft, inconsistent with every other day-count field in this matrix, including `archiveTier.moveDays`). Nothing in this spec's requirements calls for a "move on day 0" state, and no other paired field uses a different floor across parallel contexts — standardized rather than carrying an unexplained exception forward.
...
- `backup-repository-card.test.tsx` — switching `backupPath` from `direct` to `copy` stacks the Primary track above the Secondary block; switching `targetRepository` to `sobr` reveals the SOBR Design Block; switching `targetRepository` to `'vault-azure'`/`'vault-aws'` reveals the `targetRepositoryImmutableDays` field, and switching to `'sobr'` hides it.
